...
+!getPosition(slots) : not starter(_,_,_,_) 
	<- starting(slots); !getPosition(slots). 
...
+!getPosition(slots) : not finish(r2) & starter(X,Y,ZX,ZY) 
...
+!move(X,Y,ZX,ZY) : 
	finish(r2) <- .broadcast(untell,pos(X,Y)); finished(slots).
